...
const Calendario2 = () => {
  ...
  const fetchPastAppointments = async () => {
    ...
    const { data, error } = await supabase.from('citas2').select().lt('date', currentDate).order('date', { ascending: false });
  ...
  const fetchAppointments = async () => {
    const { data, error } = await supabase.from('citas2').select().order('date', { ascending: true });
  ...
  const handleTimeSelect = async () => {
    ...
    const { error } = await supabase.from('citas2').insert([
      {
        email: user.email,
        telefono,
        date: selectedDate,
  ...
  const handleEventClick = async (clickInfo) => {
    ...
    const { data, error } = await supabase.from('citas2').select().eq('id', eventId).single();
...
>
    ...
    {selectedEvent && (
        <div>
            ...
            <button
                onClick={async () => {
                    const { error } = await supabase.from('citas2').delete().eq('id', selectedEvent.id);
                    if (error) {
